...
export default async function ListingPage({ params, searchParams }: { params: Promise<{ listingId: string }>; searchParams: Promise<{ from?: string }> }) {
  ...
  return <main className="bg-marble pb-16">
    ...
    <div className="shell grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="sticky top-20 border border-border bg-white p-6"><p className="text-xs text-muted-foreground">{listing.year} · {listing.trim}</p><h1 className="font-display mt-2 text-4xl font-semibold uppercase leading-none">{listing.make} {listing.model}</h1><p className="mt-5 text-3xl font-bold">AED {listing.price.toLocaleString("en-AE")}</p><div className="mt-5"><DealPosition position={listing.dealPosition} /></div><div className="my-6 grid grid-cols-2 gap-3 border-y border-border py-4 text-sm"><span className="flex items-center gap-2"><Gauge className="size-4" />{listing.mileageKm.toLocaleString("en-AE")} km</span><span className="flex items-center gap-2"><MapPin className="size-4" />{listing.city}</span></div><div className="flex gap-2"><CompareButton listingId={listing.id} /><ShortlistButton listingId={listing.id} /></div><a href={listing.source.listingUrl} target="_blank" rel="noreferrer" className={`${buttonVariants({ variant: "signal" })} mt-3 w-full`}>Open fixture source <ExternalLink className="size-4" /></a><p className="mt-4 text-xs leading-relaxed text-muted-foreground">Fixture record only. Carveo does not inspect vehicles or guarantee seller claims.</p></aside>
      <section className="border border-border bg-white p-6"><p className="text-xs font-semibold uppercase text-muted-foreground">Price history</p><h2 className="font-display mt-2 text-3xl font-semibold uppercase">Observed fixture prices</h2><PriceHistoryChart data={listing.priceHistory} /></section>
    </div>

  ...
  return <main className="min-h-screen bg-marble pb-28">
    ...
    <div className="shell py-6">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[248px_minmax(0,1fr)]">
        ...
        <section>{results.items.length ? <div className={view === "list" ? "grid gap-4 md:grid-cols-2" : "grid gap-4 md:grid-cols-2 xl:grid-cols-3"}>{results.items.map((listing) => <VehicleCard key={listing.id} listing={listing} returnHref={returnHref} />)}</div> : <EmptyState />}{results.pageCount > 1 && <nav aria-label="Pagination" className="mt-8 flex flex-wrap gap-2">{Array.from({ length: results.pageCount }, (_, index) => { const params = toListingSearchParams({ ...query, page: index + 1 }); return <Link key={index} href={`/en-ae/cars?${params}`} className={`inline-flex size-10 items-center justify-center border ${results.page === index + 1 ? "border-obsidian bg-obsidian text-white" : "border-border bg-white"}`}>{index + 1}</Link>; })}</nav>}</section>
      </div>
    </div>
    <CompareTray />
  </main>;

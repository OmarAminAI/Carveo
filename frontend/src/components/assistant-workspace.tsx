...
export function AssistantWorkspace({ initialPrompt = "" }: { initialPrompt?: string }) {
  ...
  const [messages, setMessages] = useState(() => initialPrompt ? [
    { role: "buyer" as const, content: initialPrompt },
    { role: "assistant" as const, content: `I interpreted ${Object.keys(first.intent).length} requirement groups. Review them before searching.` },
  ] : [{ role: "assistant" as const, content: "Tell me what matters: vehicle type, budget, year, location, and any source-stated condition evidence you want to see." }]);
  ...
  const send = () => {
    ...
    setIntent((current) => ({ ...current, ...parsed.intent }));
    setMessages((current) => [...current, { role: "buyer", content: draft }, { role: "assistant", content: parsed.summary }]);
    setDraft("");
  };
  ...
  return <div className="grid min-h-[calc(100svh-64px)] bg-marble lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
    ...
    <aside className="p-5 md:p-8"><div className="sticky top-24"><p className="text-xs font-semibold uppercase text-muted-foreground">Interpreted requirements</p><h2 className="font-display mt-2 text-3xl font-semibold uppercase">Your search brief</h2><div className="mt-5 flex flex-wrap gap-2">{requirements.map(([key, value]) => <Badge key={key} className={key === "conditionPreferences" ? "border-steel text-muted-foreground" : "border-obsidian"}>{key.replace(/([A-Z])/g, " $1")}: {Array.isArray(value) ? value.join(", ") : String(value)}</Badge>)}</div>
      ...
      <div className="mt-6 flex gap-2"><Link href={intentHref(intent)} className={cn(buttonVariants({ variant: "signal" }), "flex-1")}>Search now <ArrowRight className="size-4" /></Link><Button type="button" variant="outline" size="icon" aria-label="Start over" onClick={() => { setIntent({}); setMessages([{ role: "assistant", content: "Let’s start again. What kind of vehicle do you need?" }]); }}><RotateCcw className="size-4" /></Button></div>
